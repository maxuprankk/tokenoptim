...
Same core, same protocol. But: validates license key at startup.
No valid key = tools return "unlicensed" errors.
"""

import json, re, hashlib, os, pathlib, uuid, time
...
PUBLIC_KEY_PEM = None  # À remplacer par la clé publique Stripe/RS256
...
def check_license() -> dict:
    """Vérifie la licence locale. Retourne {valid, email?, plan?, expires?}."""
    ...
    # Check machine binding (simple: hash of machine-id + user)
...
# ─── TOKEN ESTIMATION (GPT-optimized) ───

def estimate_tokens(text: str) -> int:
    s = str(text)
    code_chars = len(re.findall(r'[{}()[\]<>+=&|^~`@#]', s)) + len(re.findall(
        r'(def |class |import |from |return |print|if |else|for |while |async|await)', s))
...
def compress_system_prompt(text: str) -> str:
    ...
    politeness = re.compile(
        r'^(Certainly!|Of course|Absolutely|I\'d be happy to|Let me|I will|I\'ll|'
        r'Here is|Here are|Great question|Good question|I understand|'
        r'Je comprends|Bien sûr|Bien entendu|Absolument|Avec plaisir|'
        r'Voici|Voilà)', re.IGNORECASE
    )
    for line in lines:
        stripped = line.strip()
        if not stripped:
...
def compress_conversation(messages: list) -> dict:
    ...
    return {"compressed": compressed, "original_tokens": original_tokens, "compressed_tokens": compressed_tokens,
            "saved": saved, "compression_ratio": round((saved / original_tokens * 100), 1) if original_tokens > 0 else 0}
...
UNLICENSED = json.dumps({
    ...
    "message": "TokenSight nécessite une licence active. Va sur https://tokensight.app pour activer ton essai gratuit.",
    "action": "Lance `tokensight activate <TA_CLE>` dans ton terminal."
}, ensure_ascii=False)


def handle_optimize(arguments: dict) -> CallToolResult:
    global _stats, _license

    # Recheck license (allows hot-reload if user activates mid-session)
    _license = check_license()
    ...
    # On limite les features selon le plan
    max_compression = 0.7 if _license.get("plan") == "pro" else 0.4

    return CallToolResult(content=[TextContent(type="text", text=json.dumps({
        "cached": False,
        "compressed_messages": result["compressed"],
        "license": {"plan": _license.get("plan"), "email": _license.get("email")},
        "stats": {
...
def handle_stats(arguments: dict) -> CallToolResult:
    ...
    return CallToolResult(content=[TextContent(type="text", text=json.dumps({
        ...
        "estimated_cost_saved": f"${_stats['tokens_saved'] * 0.0000025:.2f}",
        "license": {"valid": _license["valid"], "plan": _license.get("plan"), "email": _license.get("email")},
        "message": f"TokenSight a économisé {_stats['tokens_saved']:,} tokens depuis le lancement."
    }, ensure_ascii=False))])


def handle_activate(arguments: dict) -> CallToolResult:
    """Active une licence via le MCP lui-même."""
    license_key = arguments.get("key", "").strip()
    if not license_key:
        return CallToolResult(content=[TextContent(type="text", text=json.dumps({
            "error": "Clé manquante. Utilise `tokensight activate <TA_CLE>` dans ton terminal."
        }, ensure_ascii=False))])

    # En production : validation cryptographique de la clé (RS256 signée par notre backend)
    # Pour le prototype : clé acceptée si format valide (ts_xxxx-xxxx-xxxx-xxxx)
    import re as _re
    if not _re.match(r'^ts_[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}$', license_key):
        return CallToolResult(content=[TextContent(type="text", text=json.dumps({
            "error": "Format de clé invalide.",
            "help": "La clé doit être au format ts_xxxx-xxxx-xxxx-xxxx. Va sur https://tokensight.app."
        }, ensure_ascii=False))])
    ...
    license_data = {
        ...
        "email": "user@example.com",  # À remplir par validation backend
        "plan": "pro",
        "activated_at": int(time.time()),
        "exp": int(time.time()) + 30 * 24 * 3600,  # 30 jours
    ...
    return CallToolResult(content=[TextContent(type="text", text=json.dumps({
        ...
        "trial": True,
        "expires_in_days": 30,
        "message": "TokenSight activé ! Essai gratuit de 30 jours. Profite de 2x plus de ChatGPT."
    }, ensure_ascii=False))])
...
async def list_tools_handler(req: ListToolsRequest) -> ListToolsResult:
    return ListToolsResult(tools=[
        Tool(
            ...
            description="OPTIMISE la conversation pour réduire les tokens GPT. DOIT être appelé avant chaque réponse. Licence requise.",
            inputSchema={
                "type": "object",
                "properties": {
                    "messages_json": {
                        "type": "string",
                        "description": "Conversation complète en JSON (liste de {role, content})"
                    }
                },
        ...
        Tool(
            ...
            description="Statistiques d'économies TokenSight.",
            inputSchema={"type": "object", "properties": {}}
        ),
        Tool(
            name="tokensight_activate",
            description="Active ta licence TokenSight avec une clé. Utilise ceci si optimize_conversation te dit 'LICENCE REQUISE'.",
            inputSchema={
                "type": "object",
                "properties": {
                    "key": {
                        "type": "string",
                        "description": "Clé de licence TokenSight (format: ts_xxxx-xxxx-xxxx-xxxx)"
                    }
                },
                "required": ["key"]
            }
